...
# this changes calibration method for straight line driving
# to save time no comments were changed. only the code itself
def calibrateWheelRadius():
    ...
    while not isinstance(delta_time, float):
        delta_time = input("Input the time to drive in seconds: ")
        delta_time = float(delta_time)
    ...
    for wheel_vel in wheel_velocities_range:
        ...
        uInput = float(input("What distance did the robot travel in meters? "))
        if uInput > 0:
            delta_distances.append(uInput)
            print("Recording that the robot drove {:.2f} meters at wheel speed {} for {} seconds.\n".format(delta_time,
                                                                                                    wheel_vel, delta_time))
    ...
    scale = 0
    scales_list = []
    for dis, wheel_vel in zip(delta_distances, wheel_velocities_range):
        # pass # TODO: replace with your code to compute the scale parameter using wheel_vel and delta_time
        scales_list.append( dis / (wheel_vel * delta_time))
    scale = np.mean(np.array(scales_list))
...
def calibrateBaseline(scale):
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        ...
        baseline_list.append((20*1)/(wheel_vel*delta_time))
    baseline = np.mean(np.array(baseline_list))
