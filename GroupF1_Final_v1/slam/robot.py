...
class Robot:
    ...
    def derivative_drive(self, drive_meas):
        ...
        if (ang_vel == 0):
            DFx[0, 2] = -lin_vel * dt * np.sin(th)  # x/dth
            DFx[1, 2] = lin_vel * dt * np.cos(th)  # y/dth
        else:
            R = lin_vel / ang_vel
            DFx[0,2] = -R * np.cos(th) + R * np.cos(th + ang_vel * dt) #x/dth
            DFx[1,2] = -R * np.sin(th) + R * np.sin(th + ang_vel * dt) #y/dth
    ...
    def covariance_drive(self, drive_meas):
        ...
        if (ang_vel == 0):
            Jac2[0,0] = np.cos(th) * dt #x/dv
            Jac2[0,1] = 0
            Jac2[1,0] = np.sin(th) * dt #y/dv
            Jac2[1,1] = 0
            Jac2[2,0] = 0
            Jac2[2,1] = 0
            ...
            Jac2[0,0] = (1/ang_vel)* (-np.sin(th) + np.sin(th2)) # dx/dv
            Jac2[0,1] = (-lin_vel/ang_vel**2) * (-np.sin(th) + np.sin(th2)) + (lin_vel/ang_vel) * (np.cos(th2)*dt) #dx/dw
            Jac2[1,0] = (1/ang_vel) * (np.cos(th) - np.cos(th2)) # dy/dv
            Jac2[1,1] = (-lin_vel/ang_vel**2) * (np.cos(th) - np.cos(th2)) + (lin_vel/ang_vel) * (np.sin(th2)*dt) # dy/dw
            Jac2[2,0] = 0
            Jac2[2,1] = dt
