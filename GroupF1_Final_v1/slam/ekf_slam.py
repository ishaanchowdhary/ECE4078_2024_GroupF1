...
class EKF:
    ...
    def update(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # if distance_to_marker>2.0:
            #     scale = 1.6
            # elif distance_to_marker > 1.5:
            #     scale = 1.4
            # elif distance_to_marker > 1:
            #     scale = 1.2
            ...
            R[2*i:2*i+2,2*i:2*i+2] = measurements[i].covariance * scale
        ...
        self.P = (np.eye((x).shape[0]) - K@H)@sigma_bar
    ...
    def predict_covariance(self, raw_drive_meas):
        ...
        if raw_drive_meas.left_speed == 0 and raw_drive_meas.right_speed == 0:
            Q[0:3, 0:3] = self.robot.covariance_drive(raw_drive_meas) + 0.0003 * np.eye(3)
    ...
    def add_aruco_marker(self, ground_truth):
        for aruco_pos, id in zip(ground_truth,range(len(ground_truth))):
            aruco_x, aruco_y = aruco_pos
            lm = np.array([aruco_x,aruco_y]).reshape(2,-1)

            self.taglist.append(id+1)
            self.markers = np.concatenate((self.markers,lm),axis=1)
    ...
    def add_landmarks(self, measurements):
        ...
        for lm in measurements:
            ...
            # Set the initial one as very accurate
