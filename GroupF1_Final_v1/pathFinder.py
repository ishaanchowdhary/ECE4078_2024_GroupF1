...
def create_grid_d_star(fruits, arucos, safe_dis, dest_dis, size = 3, resolution = 10 ):
    ...
    for i in range(len(grid[:,0])):
        for j in range(len(grid[0,:])):
            ...
            if (i ==0 or i == size*resolution-1):
                grid[i][j] = 1
                #plt.scatter(step[j], step[i], color='red', s=10)
    ...
    return grid, step, plt, ax, destination_nodes

def update_map(grid, step, paths, plt, ax):
    for path in paths:
        #print(path)
        for node in path:
...
def special_nodes(array, arucos, plt, min):
    # array [fruit, x, y]
...
def special_nodes_new(array, arucos, plt, min):
    # array [fruit, x, y]
...
def main():
    ...
    file_path = 'slam_generated_map.txt'   #slam_generated_map
    ...
    #print(fruit_Nodes)
    #print(aruco_Nodes)
    grid, step, plot, axes, destination_nodes = create_grid_d_star(fruit_Nodes, aruco_Nodes, safe_dis, dest_dis, 3, res)
    #print(grid[15][15])
    #print(step)
    #result = special_nodes(destination_nodes, aruco_Nodes, plot, min_dis_aruco)
    result = special_nodes_new(destination_nodes, aruco_Nodes, plot, min_dis_aruco)
    start = [15, 15]
    goal = [[15, 15]]
    ...
    for i in range(len(goal)-1):
        ...
        #print(goal[i+1])
        new_safe = safe_dis
        grid, step, plot, axes, destination_nodes = create_grid_d_star(fruit_Nodes, aruco_Nodes, new_safe, dest_dis,
                                                                       3, res)
    ...
    #update_map(grid, step, plot_path, plot, axes)
    #print(plot_path)
