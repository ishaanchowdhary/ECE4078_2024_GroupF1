...
class Operate:
    def __init__(self, args):
        ...
        self.bg = pygame.image.load('pics/jangus.png')
    ...
    def update_slam(self, drive_meas):
        ...
        if self.request_recover_robot:
            ...
            #print(f"Got to update slam")
            self.ekf.predict(drive_meas)
            self.ekf.add_landmarks(lms)
            p_robot = self.ekf.P[0:2, 0:2]
            axes_len, angle = self.ekf.make_ellipse(p_robot)
            error = np.linalg.norm(axes_len)
            if (error > 0.4 ): #and len(lms) > 1
                self.ekf.update(lms)
    ...
    def draw(self, canvas):
        ...
        canvas.blit(ekf_view, (2 * h_pad + 384, v_pad))
        ...
        self.put_caption(canvas, caption='SLAM', position=(2 * h_pad + 384, v_pad))
        self.put_caption(canvas, caption='Path',
                         position=(h_pad, 240 + 2 * v_pad))
        self.put_caption(canvas, caption='PiBot Cam', position=(h_pad, v_pad))
        self.notification = 'Cooking...'
        notifiation = TEXT_FONT.render(self.notification,
                                       False, text_colour)
        canvas.blit(notifiation, (2 * h_pad + 384 + 5, 596))

        # load planned path as bytes and display
        fid = open('gridPathPlanner.txt', 'rb')
        rawData = fid.read()
        fid.close()
        surf = pygame.image.fromstring(rawData, (384,288), "RGB")
        canvas.blit(surf, (h_pad,240 + 2 * v_pad))

        time_remain = self.count_down - time.time() + self.start_time
        if time_remain > 0:
            time_remain = f'Count Down: {time_remain:03.0f}s'
        elif int(time_remain) % 2 == 0:
            time_remain = "Xue Hua Piao Piao"
        else:
            time_remain = ""
        count_down_surface = TEXT_FONT.render(time_remain, False, (50, 50, 50))
        canvas.blit(count_down_surface, (2 * h_pad + 384 + 5, 530))
        return canvas
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('ECE4078 2024 Group F1')
